class PhotosController < ApplicationController
  ...
  before_action :set_ad, only: %i[create destroy]
  ...
  def create
    ...
    if @new_photo.save
      # Если фотографию удалось сохранить, редирект на редактирование объявления с сообщением
      redirect_to edit_ad_path(@ad), notice: I18n.t('controllers.photos.created')
    else
  ...
  def destroy
    ...
    # И в любом случае редиректим его на редактирование объявления
    redirect_to edit_ad_path(@ad), message
  end

  private

  # Так как фотография — вложенный ресурс, то в params[:ad_id] рельсы
  # автоматически положат id объявления, которому принадлежит фотография,
